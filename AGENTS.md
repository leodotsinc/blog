...
- `.github/maintenance.json` and `scripts/maintenance-yarn-admission.py` define the closed Blog preparation envelope using the scheduler's numeric identities, canonical receipt/request hashes and bounded TTL. The dedicated `maintenance.yml` workflow invokes a read-only observation receiver under the existing production concurrency group, with seven-day request-deduplication artifacts. It authenticates the scheduler identity, corroborates the requested baseline against the latest verified Blog release asset and checksum, and independently collects immutable Git/PR/CI/security/functional evidence. It never downloads or executes candidate scripts. The configuration remains disabled with missing policy/code/host qualification pins, and every result denies preparation/deployment authorization. The final host must revalidate live production and policy; no GitHub artifact alone proves runtime. Review-image receipts (CI, version0.1.0) cannot qualify a subsequently rebuilt release image. A future mutating receiver/release path and its host qualification remain required; the ordinary `maintenance-guard.py` is unchanged. Existing cumulative adoption changes must be released through a separately reviewed operational bootstrap; the classifier has no bootstrap exception. Run `python3 -B -m unittest discover -s scripts -p 'test_release*.py'` for the source/entry contract tests.
