...
- `/api/health` não chama Spotify; `/api/version` expõe apenas versão, commit e build. Os testes de imagem usam ambiente isolado sem secrets. HTTP/identidade não substituem verificação visual, disponibilidade do Spotify ou restore. Hero, conteúdo, UI, integração Spotify e Dependabot permanecem fora deste lote.
