...
- `.github/maintenance.json` and `scripts/maintenance-yarn-admission.py` define the closed Blog preparation envelope using the scheduler's numeric identities, canonical receipt/request hashes and bounded TTL. This entry is intentionally inactive and every result denies preparation/deployment authorization; no dispatch workflow, merge/build capability or bypass of `maintenance-guard.py` is introduced. The host-qualified receiver, trusted production-receipt collector, calendar authorization and image/functional receipts remain required integration work. Existing cumulative adoption changes must be released through a separately reviewed operational bootstrap; the classifier has no bootstrap exception. Run `python3 -B -m unittest discover -s scripts -p 'test_release*.py'` for the source/entry contract tests.
