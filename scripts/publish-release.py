...
IMAGE = re.compile(r"ghcr\.io/leodotsinc/blog@sha256:[0-9a-f]{64}\Z")
...
def verify_published_asset(repository, tag, revision, published, *, expected_bytes=None):
    ...
    require(manifest["service"] == "blog" and manifest["compose_project"] == "leodotsdev"
...
def publish(path, environment=None):
    ...
    notes = (f"Deployed release {tag}\n\nCommit: `{revision}`\n\nImage: `{manifest['image']}`\n\n"
             ...
             "This does not certify Spotify provider availability, visual acceptance or disaster recovery.\n")
    with tempfile.NamedTemporaryFile(mode="w", encoding="utf8", suffix=".md", prefix="blog-release-") as note_file:
