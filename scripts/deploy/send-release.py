...
def main():
    ...
    if (manifest['service'] != 'blog' or manifest['compose_project'] != 'leodotsdev'
        or manifest['application_kind'] != 'first_party'
        or manifest['source_repository'] != 'https://github.com/leodotsinc/blog'
        or not re.fullmatch(r'ghcr\.io/leodotsinc/blog@sha256:[0-9a-f]{64}', manifest['image'])
        or manifest['app_version'] != manifest['release_version']
        or manifest['release_version'] != os.environ['APP_VERSION']
    ...
    command = ['ssh', '-T', '-o', 'BatchMode=yes', '-o', 'StrictHostKeyChecking=yes',
               ...
               '-o', 'ServerAliveCountMax=4', '-i', str(Path.home()/'.ssh/blog_deploy'),
               '-p', port, user+'@'+host, 'blog-release']
    ...
    if sys.argv[1] == 'deploy':
        ...
        if ({k:v for k,v in deployed.items() if k != 'deployment'} !=
            {k:v for k,v in manifest.items() if k != 'deployment'}
